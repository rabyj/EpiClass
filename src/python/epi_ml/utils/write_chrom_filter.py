...
def compute_second_arm_region(
    centromere_end, chromsize, width=303 * HUNDRED_KB, resolution=HUNDRED_KB
):
...
def main():
    ...
    for name, size in chroms.items():
        begin, end = compute_second_arm_region(
            centromere_end=centro[name][1], chromsize=size
        )
